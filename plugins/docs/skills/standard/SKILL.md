...
prose that summarises a machine-readable artifact — findings in a record, rows in a table, entries in
a log — is computed, never typed:** the repository ships the script that recomputes it, and that
script is wired as a `checks` entry on the review's docs lens so the lens receives the fact instead of
re-deriving it. **A decision that retires or renames a term sweeps the whole document in the same
change and adds the term to that script** — the fix for a stale term is never the cited line alone,
because the next reader finds the next instance (measured: eight review attempts on one design
change, each finding two more). The structure rules — index agreement, required fields, internal links — have no such exemption: an exemption from being correct would not be an exemption, it would be a hole.
